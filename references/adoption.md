...
If a document-like record must remain in a database for product reasons, expose an `oxi-document/1` import/export adapter only when cross-app document interchange is actually required. Do not force the database's internal schema to mimic a file.
...
3. Parse the envelope with the frozen constrained grammar, then parse the body as `oxi-djot/1` rather than "whatever Markdown this library accepts."
...
2. Add an OXI Document Reader without changing existing files.
3. Make invalid or unsupported `.djot` files visible instead of silently skipping them.
4. Write `oxi-document/1` for newly created documents while retaining legacy readers.
...
- Link clicks, embeds, tasks, query blocks, and asset URLs must go through OXI resolvers; do not hand custom URI schemes directly to a browser engine.
...
> User-authored durable documents in this repository follow OXI Document 1. Use the `oxi-document-standard` skill before changing their storage, editor, renderer, discovery, linking, asset, or migration behavior. Repository documentation is out of scope.
