...
description: Apply or audit the Portable Document Contract when an app stores, reads, edits, indexes, migrates, links, embeds, or renders durable user-authored notes or documents. Do not apply it to repository documentation, source-code Markdown, logs, caches, generated previews, or export-only files.
---
...
- Treat `pdc-document/1` and `pdc-djot/1` as the default canonical write format. Do not introduce an app-specific canonical Markdown, HTML, JSON, or rich-text format for in-scope documents.
